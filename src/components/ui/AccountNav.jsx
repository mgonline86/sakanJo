...
const AccountNav = () => {
  ...
  const linkClases = (type = null) => {
    let classes =
      'flex justify-center gap-1 py-2 px-6 rounded-full';
    if (type === subpage) {
  ...
  return (
    <nav className="mt-24 mb-8 flex w-full justify-center gap-2">
      ...
      <Link className={linkClases('bookings')} to={"/account/bookings"}>
        ...
        <span className='hidden md:block'>{t("myBooking")}</span>
      </Link>
      <Link className={linkClases('places')} to={'/account/places'}>
        ...
        <span className='hidden md:block'>{t("myPosts")}</span>
      </Link>
    </nav>
  );
};
